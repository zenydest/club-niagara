 ...
 *
 * El ciclo es: creamos la orden apuntando a la terminal, la terminal la levanta,
 * el cliente paga ahí y nosotros esperamos el resultado.
 *
...
interface CobroPointState {
  // Terminal en uso por esta caja
  terminales: TerminalPos[];
  terminalId: string | null;
  cargandoTerminales: boolean;

  // Cobro en curso
  estado: EstadoCobroPos;
  referencia: string | null;
  orden: OrdenPointPos | null;
  error: string | null;
  ...
  persist(
    (set, get) => ({
      ...
      cargarTerminales: async () => {
        ...
        try {
          const data = await api.get<{ terminales: TerminalPos[] }>(
            "/point/terminales",
            staff.localId
          );

          // Solo sirven las que están en PDV: las demás no reciben órdenes por API.
          ...
          set({
            cargandoTerminales: false,
            error: err instanceof Error ? err.message : "No se pudieron cargar las terminales",
          });
        }
      },
      ...
      cobrar: async ({ ventaId, monto, descripcion }) => {
        ...
        try {
          const { orden } = await api.post<{ orden: OrdenPointPos }>(
            "/point/cobros",
            {
              ventaId,
              terminalId,
          ...
          return await esperarResultado(ventaId, staff.localId, set);
        } catch (err) {
          const mensaje =
      ...
      cancelar: async () => {
        ...
        try {
          await api.post(`/point/cobros/${referencia}/cancelar`, {}, staff.localId);
...
function esperarResultado(
  ...
  return new Promise((resolve) => {
    const consultar = async () => {
      ...
      try {
        const { orden } = await api.get<{ orden: OrdenPointPos }>(
